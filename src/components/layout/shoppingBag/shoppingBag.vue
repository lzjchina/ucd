<template>
  <div class="ShoppingBagBox">
    ...
    <div class="box packageBox" v-if="packageAll.length > 0">
      <table class="package" v-for="(packageItem, index) in packageAll">
        <thead class="packageHead">
          <tr style="display:flex;justify-content:space-between;width:100%;">
            <td class="packageType">{{packageItem.packageType}}</td>
            <td class="packageRemove" @click="removePackage(index)"><i class="removeIco"></i>Remove</td>
          </tr>
        </thead>
      </table>
    </div>
        ...
        <div class="itemAttr">
          <div class="itemNumberBox">
            <div class="itemNumber">
              <button class="itemNumberBtn" :disabled="disabled" @click="reduceChangeNum(index, item.itemNumber)">-</button>
              <i style="font-style:normal;" v-on:input="itemNumberChange">{{item.itemNumber}}</i>
              <button class="itemNumberBtn" @click="addChangeNum(index, item.itemNumber)">+</button>
            </div>
